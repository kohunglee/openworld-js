        ...
        keyEvent : (e) => {
            ...
            if(G.disListen() === false && document.activeElement.tagName !== 'INPUT') {  // 仅在编辑器打开且未激活 input 有效
                ...
                if (action) {  //键盘操作物体
                    ...
                    if(action.type === 'vertical'){  // Y 轴移动单独处理
                        const objPosY = document.getElementById('objPosY');
                        var yDelta = action.dir * step;
                        if(magicNum.value){
                            yDelta = action.dir > 0 ? Number(magicNum.value) : -Number(magicNum.value);
                            G.clearMagicNum();
                        }
                        objPosY.value = G.f(+objPosY.value + yDelta);
                        G.modelUpdate();
                        return 0;
                    }
