    ...
    ccgxkObj.hooks.on('pointer_lock_click', function(obj, e){
        if(ccgxkObj.centerPointColorUpdatax || e.button === 2){  
            if(ccgxkObj.hotPoint >= 0 && e.button !== 2) {  // 如果有热点，单击热点后，触发热点事件
                ...
                // music('closePoint');
                drawCenterPoint(canvas, ccgxkObj, true, 1);
            ...
            ccgxkObj.centerPointColorUpdatax = setInterval(() => {
                ...
                drawCenterPoint(canvas, ccgxkObj, false, 3);
                console.log('kkk');
            }, 100);
...
function openPoint(){
    ...
    const W = ccgxkObj.W;
    if(W.makeFBOSucess !== true){ W.makeFBO() }
    drawCenterPoint(canvas, ccgxkObj, false, 4);
    // music('openPoint');
    if(ccgxkObj.centerPointColorUpdatax === null){
        ccgxkObj.centerPointColorUpdatax = setInterval(() => {
            // if(myHUDModal.hidden === false){ return 0;}  // 如果显示了模态框，则暂停
            drawCenterPoint(canvas, ccgxkObj, false, 5);
            console.log('ggg');
        }, 100);
    }
    ccgxkObj.mainCamera.pos = {x:0, y:0.9, z:-0.8};
...
// 绘制屏幕中心的点
function drawCenterPoint(canvas, thisObj, isClear, id){
