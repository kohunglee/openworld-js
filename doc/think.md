...
## 28. AI 边界修正：AI 应该生成铲子，不应该替用户堆成品

昨天的方案里把 AI 写成“自动整理、自动选址、自动摘要、自动生成空间结构、自动把内容变成展馆”。这个方向需要收紧。它商业上看起来顺，但产品直觉上有危险：如果 AI 直接把大量内容自动堆成一座“知识宫殿”，用户很可能不会珍惜。

这个担心不是保守，而是很关键的产品判断。AI 文字已经证明了同一件事：当内容太容易生成时，内容本身会迅速变廉价。用户不会因为生成了更多文字而更爱它，反而会因为太多、太快、太像垃圾而厌烦。手机照片也是同理，拍照成本降到接近 0 后，几万张照片反而让记忆变得更难整理、更难回看。

所以 openworld-js 和 AI 的结合，不能走“AI 自动生成一个繁重知识宫殿”的路线。真正值得长期留下来的内容，往往是用户一点一点自己摆上去、修改过、选择过、删掉过、重新命名过的东西。空间感的价值来自投入感，而不是自动化产量。

更准确的边界应该是：

- AI 可以生成建筑、房间、空白展板、路线模板、布局建议。因为这像生成铲子、脚手架、工作台，降低的是建造成本。
- AI 可以帮用户把一段内容压缩成候选卡片，但最终哪张卡片挂上墙、放在哪个房间、用什么标题，应该让用户确认。
- AI 可以建议“这 5 条内容可能属于同一主题”，但不应该默认把 500 条内容全塞进一个世界。
- AI 可以做导入时的去重、摘要、标签、图片说明，但产品上要鼓励少量精选，而不是鼓励无限生成。
- AI 可以给出“下一步你可以摆什么”的建议，但不要替用户完成整个空间。

因此，产品表达要从 `AI generates your knowledge palace` 改成类似：

- `Build a personal knowledge space with AI-assisted tools.`
- `Turn selected notes into a walkable room, one board at a time.`
- `AI helps you prepare the materials. You decide what deserves a wall.`
- `Not another AI content dump. A place for the few things worth keeping.`

这也会影响 MVP。第一版不应该做“一键导入 1000 篇文章生成巨大宫殿”。更好的 MVP 是：

1. 用户选择 5 到 20 条内容。
2. AI 只生成候选标题、摘要、图片说明和推荐房间。
3. 用户逐块确认、移动、删除、改名。
4. 系统把“亲手放上墙”的动作做得顺滑、有反馈、有保存感。
5. 世界默认鼓励小而精，而不是大而全。

这样 AI 仍然重要，因为现在是 AI 时代，用户也期待工具有智能。但智能不应该抢走人的所有权感。AI 负责省力，用户负责选择；AI 负责准备材料，用户负责把东西变成自己的空间。这个边界一旦守住，openworld-js 才不会变成另一个自动生成垃圾内容的机器。
