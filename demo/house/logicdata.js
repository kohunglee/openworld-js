...
function logicData(){  // 
    ...
    if(k.isLogicAdd === '1'){
        ...
        if(true){
            ...
            const bookSuby = cubeDatas[k.bookS.floor2.bookSub.first].y;
        ...
        if(true){
            // 初始化
            k.bookS.floor2.bookSub.cg = {};  // 长柜的假书替身
            k.bookS.floor2.bookSub.cg.first = 50;
            cubeDatas[k.bookS.floor2.bookSub.cg.first].st = 1;  // 无物理

            // 二楼 第一统柜的假书 参考假书，后续的书都以此为基准
            k.bookS.floor2.bookSub.cg.tgRef = offset([
                k.bookS.floor2.bookSub.cg.first,
            ], -0.352, 7, 'y');
            k.bookS.floor2.bookSub.cg.tgRef.push(k.bookS.floor2.bookSub.cg.first);

            // // 辅助计算值，计算相对位置
            const bookSubx = cubeDatas[k.bookS.floor2.bookSub.cg.first].x;
            const bookSuby = cubeDatas[k.bookS.floor2.bookSub.cg.first].y;
            const bookSubz = cubeDatas[k.bookS.floor2.bookSub.cg.first].z;
            ...
            k.bookS.floor2.cdbook.dire1.forEach((v, i) => {
                ...
                const x_diff = bookSubx - curr_x;
                k.bookS.floor2.bookSub['s'+ v] = offset([
                    ...k.bookS.floor2.bookSub.cg.tgRef,
                ], x_diff, 2)
            });
            ...
            // 清空参考假书模型，防止看起来很奇怪
            k.bookS.floor2.bookSub.cg.tgRef = null;  
        }

        // 假书 廊柜
        if(true){
            // 初始化
            k.bookS.floor2.bookSub.lg = {};  // 长柜的假书替身
            k.bookS.floor2.bookSub.lg.first = 0;
            cubeDatas[k.bookS.floor2.bookSub.lg.first].st = 1;  // 无物理

            // 二楼 第一统柜的假书 参考假书，后续的书都以此为基准
            k.bookS.floor2.bookSub.lg.tgRef = offset([
                k.bookS.floor2.bookSub.lg.first,
            ], -0.345, 6, 'y');
            k.bookS.floor2.bookSub.lg.tgRef.push(k.bookS.floor2.bookSub.lg.first);

            // // 辅助计算值，计算相对位置
            const bookSubx = cubeDatas[k.bookS.floor2.bookSub.lg.first].x;
            const bookSuby = cubeDatas[k.bookS.floor2.bookSub.lg.first].y;
            const bookSubz = cubeDatas[k.bookS.floor2.bookSub.lg.first].z;
            ...
            k.bookS.floor2.LGbook.dire1.forEach((v, i) => {
                ...
                const x_diff = bookSubx - curr_x;
                k.bookS.floor2.bookSub['s'+ v] = offset([
                    ...k.bookS.floor2.bookSub.lg.tgRef,
                ], x_diff, 2)
            });
